...
using Nanook.GrindCore.DeflateZLib.DeflateManaged;
    ...
    public partial class ZipArchiveEntry
    {
        ...
        internal static string ParseFileName(string path, ZipVersionMadeByPlatform madeByPlatform)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return madeByPlatform == ZipVersionMadeByPlatform.Unix ? GetFileName_Unix(path) : GetFileName_Windows(path);
            else
                return madeByPlatform == ZipVersionMadeByPlatform.Windows ? GetFileName_Windows(path) : GetFileName_Unix(path);
        }
        ...
        internal ZipArchiveEntry(ZipArchive archive, string entryName)
        {
            ...
            FullName = entryName; //this sets _storedEntryNameBytes
        ...
        internal bool LoadLocalHeaderExtraFieldAndCompressedBytesIfNeeded()
        {
            ...
            if (!_everOpenedForWrite && _originallyInArchive)
            {
                ...
                int MaxSingleBufferSize = int.MaxValue;
