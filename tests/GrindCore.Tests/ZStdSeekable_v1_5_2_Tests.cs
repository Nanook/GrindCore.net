...
namespace GrindCore.Tests
{
    public sealed class ZStdSeekable_v1_5_2_Tests
    {
        ...
        public void SeekableDecoder_EdgeCases_VerySmallData()
        {
            ...
            using (var decoder = new ZStdSeekableDecoderV1_5_2(compressed))
            {
        ...
        private byte[] CompressSeekableUsingStream(byte[] inputData, int frameSize, int compressionLevel)
        {
            ...
            {
                using (var seekableStream = new ZStdSeekableStream(memoryStream, maxFrameSize: frameSize, compressionLevel: compressionLevel, leaveOpen: true, version: CompressionVersion.ZStd(ZStdVersion.v1_5_2)))
                {
